class ForceLineOrdering < ActiveRecord::Migration
  def change
    ...
    line_data.each do |line|
      line_to_update = Line.where(line_type: line[:line_type], name: line[:name]).first
      line_to_update.update_attributes(position: line[:position]) if line_to_update.present?
    end
  end
end
