...
describe BuckettedOutageDuration do
  ...
  describe 'fetch' do
    ...
    it 'adds outages from the same line' do
      ...
      expect(subject.fetch[Line.first.id]).to eq({
        1451606400 => 0,
        1451692800 => 0,
        1451779200 => 0,
        1451865600 => 0,
  ...
  describe 'bucketted_durations_for' do
    # Global, integration spec
    it 'returns buckets rounded to the nearest start' do
      ...
      buckets = subject.send(:bucketted_durations_for, Outage.new(boundaries: start..finish))
      expect(buckets).to eq({
        1451606400 => 0,
        1451692800 => 0,
        1451779200 => 0,
